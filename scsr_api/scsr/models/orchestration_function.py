...
class OrchestrationFunctionDB(db.Document):
    ...
    gamefication = db.ReferenceField(BehaviorDB, db_field="gamefication", required=True) # pylint: disable=no-member
    ...
    def to_obj(self):
        return OrchestrationFunctionAO.from_db(self)
    ...
    def __semi_hash__(self):
        ...
        """
        the_ret=(self.external_id if self.external_id else "None")+" - "+self.interactivity.__semi_hash__()+" - "+self.mechanical.__semi_hash__()+" - "+" - "+self.gamefication.__semi_hash__()+" - "+str(self.updated)
    ...
    @staticmethod
    def __persist__(pf):
        if not isinstance(pf,OrchestrationFunctionAO):
            raise TypeError("ERROR: Argument is not a valid OrchestrationFunctionAO object.")
        #get the DB object of the function
        to_save=OrchestrationFunctionDB.objects.filter(external_id=pf.external_id).first() # pylint: disable=no-member
        if not to_save:
            raise RuntimeError("ERROR: Unable to retrieve reference to persist - OrchestrationFunctionDB - "+pf.external_id)
        ...
        gameDB=BehaviorDB.objects.filter(external_id=pf.gamefication.external_id).first() # pylint: disable=no-member
        ...
        try:
            ...
            the_log.log("ERROR: Orchestration Function not persisted in _persist_.")
        finally:
            return to_save
    ...
    def __create_persistence__():
        """ Create the persistence object for the orchestration function. (use as constructor)
            ...
            Raises:
                ...
                RuntimeError -- Error creating OrchestrationFunctionDB - external_id not returned
                RuntimeError -- Error creating OrchestrationFunctionDB - general error
            
            Returns:
                OrchestrationFunctionDB -- The DB object persisted (with valid external_id and oid)
            """

        to_save=OrchestrationFunctionDB()
        mechanical_=None
        interactivity_=None
        gamefication_=None
        not_err=set()
        ...
        try:
            ...
            not_err=set([mechanical_,interactivity_,gamefication_])
        ...
        try:
            ...
            if not to_save.external_id:
                ...
                raise RuntimeError("ERROR: Error creating OrchestrationFunctionDB object. Persistence did not return an external_id")
        except RuntimeError as e:
            #there goes all the work!
            mechanical_.delete()
            interactivity_.delete()
            gamefication_.delete()
            raise RuntimeError("ERROR: Error creating OrchestrationFunctionDB object. - "+str(to_save),e)
        return to_save
    ...
    def pre_save(cls, sender, document, **kwargs):
        ...
        Arguments:
            sender {OrchestrationFunctionDB} -- The sender of the signal
            document {OrchestrationFunctionDB} -- The instance of the document
        """
        if(not document.external_id):
            document.external_id=str(uuid4())

signals.pre_save.connect(OrchestrationFunctionDB.pre_save, sender=OrchestrationFunctionDB)

#As with Behaviors, FunctionAO objects must not be created for manipulation. A FunctionAO may always have an ExternalID.
class OrchestrationFunctionAO(abc.MutableSet):
    
    def __init__(self, interactivity_=None, mechanical_=None, gamefication_=None):
        ...
        Keyword Arguments:
            ...
            gamefication_ {BehaviorAO} -- BehaviorAO object referring to gamefication (default: {None})
        ...
        if not isinstance(gamefication_,BehaviorAO):
            raise TypeError("ERROR: Argument/Parameter is not a valid BehaviorAO - Gamefication.")
        self.interactivity = interactivity_
        self.mechanical = mechanical_
        self.gamefication = gamefication_
        self.external_id = None
        self.updated=""
        self.created=""

        #set indicates what elements are in the function (the union of all behaviors)
        self.elements = set((self.interactivity | self.mechanical | self.gamefication).elements)
    ...
    def quantify(self):
        return {
            ...
            "gamefication": self.gamefication.quantify(),
    ...
    @staticmethod
    def from_db(db_obj):
        if(not isinstance(db_obj,OrchestrationFunctionDB)):
            raise TypeError("Parameter is not a valid OrchestrationFunctionDB object.")
        retorno = OrchestrationFunctionAO(db_obj.interactivity.to_obj(),db_obj.mechanical.to_obj(),db_obj.gamefication.to_obj())
    ...
    @staticmethod
    def create_function():
        the_db=OrchestrationFunctionDB.__create_persistence__()
    ...
    def to_json(self):
        retorno={
            ...
            "gamefication":self.gamefication.to_json()
        }
        return retorno
    ...
    def __semi_hash__(self):
        """
            ...
            Returns the string computed.
        """
        the_ret=(self.external_id if self.external_id else "None")+" - "+self.interactivity.__semi_hash__()+" - "+self.mechanical.__semi_hash__()+" - "+" - "+self.gamefication.__semi_hash__()
    ...
    def discard(self, element):
        ...
        self.gamefication.discard(element)
    ...
    def __or__(self, other):
        ...
        retorno=self.__class__(self.interactivity | other.interactivity, self.mechanical | other.mechanical, self.gamefication | other.gamefication)
        return retorno
    ...
    def __radd__(self,other):
        if other:
            ...
            other=OrchestrationFunctionAO()
        retorno = OrchestrationFunctionAO(self.interactivity+other.interactivity, self.mechanical+other.mechanical, self.gamefication+other.gamefication)
    ...
    def diff(self,other):
        ...
        retorno={
            ...
            "gamefication":self.gamefication.diff(other.gamefication)
        }
        return retorno

    @staticmethod
    def s_diff(one,other):
        if not isinstance(other,OrchestrationFunctionAO) or not isinstance(one,OrchestrationFunctionAO):
            raise TypeError("ERROR: Trying to sum two different, non related objects")
        retorno={
            "interactivity":one.interactivity.diff(other.interactivity),
            "mechanical":one.mechanical.diff(other.mechanical),
            "gamefication":one.gamefication.diff(other.gamefication)
        }
        return retorno
    ...
    def revert_diff(self,difdata):
        ...
        self.gamefication.revert_diff(difdata['gamefication'])
        return self
    ...
    def get_function(external_id_):
        ...
        db_obj=OrchestrationFunctionDB.objects.filter(external_id=external_id_).first() # pylint: disable=no-member
        if not db_obj:
            raise RuntimeError("Error: No persistence data found for orchestration function id: "+external_id_)
        return db_obj.to_obj()

    def save(self):
        """ Persists the orchestration function data.
        ...
        try:
            ...
            self.gamefication=self.gamefication.save()
        except:
            if self.gamefication!=gamef:
                self.gamefication=gamef
            if self.mechanical!=lud:
                self.mechanical=lud
            if self.interactivity!=inter:
                self.interactivity=inter
            raise RuntimeError("ERROR: Some of the orchestration function behaviors were not saved. ")
        #try saving the function
        try:
            OrchestrationFunctionDB.__persist__(self)
        except:
            raise RuntimeError("ERROR: Not able to save the Orchestration Function: "+self.external_id)
    ...
    def discard_mechanical_element(self, element):
        ...
        if element in self.mechanical:
            ...
            if (not ((element in self.interactivity) or (element in self.gamefication))):
                self.elements.discard(element)
    ...
    def discard_interactivity_element(self, element):
        ...
        if element in self.interactivity:
            self.interactivity.discard(element)
            #need to check in the others to verify if remove from the set.
            if (not ((element in self.mechanical) or (element in self.gamefication))):
                self.elements.discard(element)

    def add_gamefication_element(self, element):
        if not isinstance(element,ElementAO):
            raise TypeError("ERROR: Argument not a valid ElementAO object.")
        self.gamefication.add(element)
        self.elements.add(element)

    def discard_gamefication_element(self, element):
        if not isinstance(element,ElementAO):
            raise TypeError("ERROR: Argument not a valid ElementAO object.")
        if element in self.gamefication:
            self.gamefication.discard(element)
            #need to check in the others to verify if remove from the set.
    ...
    def set_gamefication(self,gamefication_):
        if not isinstance(gamefication_,BehaviorAO):
            raise TypeError("ERROR! Argument is not a valid BehaviorAO object")
        if gamefication_.behavior_type!="GAMEFICATION":
            raise TypeError("ERROR! Assigning a Non gamefication Behavior Type to Behavior Slot")
        #The set function does not properly replace the Behavior due to the External_id
        # it replaces only the Behavior Element List. This will be processed in the Diff when persisting.
        self.gamefication.elements=gamefication_.elements
    ...
    def p_add_gamefication_element(self, element):
        if not isinstance(element,ElementAO):
            raise TypeError("ERROR: Argument not a valid ElementAO object.")
        self.gamefication.add(element)
        self.elements.add(element)
        self.gamefication.save()

    def p_discard_gamefication_element(self, element):
        if not isinstance(element,ElementAO):
            raise TypeError("ERROR: Argument not a valid ElementAO object.")
        if element in self.gamefication:
            self.gamefication.discard(element)
    ...
    def p_set_gamegfication(self,gamefication_):
        if not isinstance(gamefication_,BehaviorAO):
            raise TypeError("ERROR! Argument is not a valid BehaviorAO object")
        if gamefication_.behavior_type!="GAMEFICATION":
            raise TypeError("ERROR! Assigning a Non gamefication Behavior Type to Behavior Slot")
    ...
    def copy(self):
        ...
        """
        to_ret=OrchestrationFunctionAO(self.interactivity.copy(), self.mechanical.copy(), self.gamefication.copy())
        return to_ret
    ...
    def issuperset(self,other):
        ...
        gam_superset=self.gamefication.issuperset(other.gamefication)
    ...
    def isdisjoint(self,other):
        ...
        gam_inter=self.gamefication & other.gamefication
    ...
    def pop(self):
        ...
        if(el in self.gamefication):
            beh.append("gamefication")
        self.discard(el)
        return {
            "element": el,
            "behavior": beh
