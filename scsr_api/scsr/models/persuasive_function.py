...
class PersuasiveFunctionDB(db.Document):
    ...
    def __semi_hash__(self):
        ...
        """
        the_ret=(self.external_id if self.external_id else "None")+" - "+self.interactivity.__semi_hash__()+" - "+self.ludic.__semi_hash__()+" - "+" - "+self.gamefication.__semi_hash__()+" - "+str(self.updated)
    ...
    def __persist__(pf):
        if not isinstance(pf,PersuasiveFunctionAO):
            raise TypeError("ERROR: Argument is not a valid PersuasiveFunctionAO object.")
        #get the DB object of the function
        to_save=PersuasiveFunctionDB.objects.filter(external_id=pf.external_id).first() # pylint: disable=no-member
        if not to_save:
            raise RuntimeError("ERROR: Unable to retrieve reference to persist - PersuasiveFunctionDB - "+pf.external_id)
        ...
        try:
            ...
            the_log.log("ERROR: Persuasive Function not persisted in _persist_.")
        finally:
            return to_save
    ...
    def __create_persistence__():
        ...
        try:
            ...
            if not to_save.external_id:
                ...
                raise RuntimeError("ERROR: Error creating PersuasiveFunctionDB object. Persistence did not return an external_id")
        except RuntimeError as e:
            #there goes all the work!
            ludic_.delete()
            interactivity_.delete()
            gamefication_.delete()
            raise RuntimeError("ERROR: Error creating PersuasiveFunctionDB object. - "+str(to_save),e)
        return to_save
...
signals.pre_save.connect(PersuasiveFunctionDB.pre_save, sender=PersuasiveFunctionDB)

#As with Behaviors, FunctionAO objects must not be created for manipulation. A FunctionAO may always have an ExternalID.
class PersuasiveFunctionAO(abc.MutableSet):
    
    def __init__(self, interactivity_=None, ludic_=None, gamefication_=None):
        ...
        self.elements = set((self.interactivity | self.ludic | self.gamefication).elements)
    ...
    @staticmethod
    def from_db(db_obj):
        if(not isinstance(db_obj,PersuasiveFunctionDB)):
            raise TypeError("Parameter is not a valid PersuasiveFunctionDB object.")
        retorno = PersuasiveFunctionAO(db_obj.interactivity.to_obj(),db_obj.ludic.to_obj(),db_obj.gamefication.to_obj())
    ...
    @staticmethod
    def create_function():
        the_db=PersuasiveFunctionDB.__create_persistence__()
    ...
    def __semi_hash__(self):
        """
            ...
            Returns the string computed.
        """
        the_ret=(self.external_id if self.external_id else "None")+" - "+self.interactivity.__semi_hash__()+" - "+self.ludic.__semi_hash__()+" - "+" - "+self.gamefication.__semi_hash__()
    ...
    def __or__(self, other):
        ...
        retorno=self.__class__(self.interactivity | other.interactivity, self.ludic | other.ludic, self.gamefication | other.gamefication)
        return retorno
    ...
    def __radd__(self,other):
        if other:
            ...
            other=PersuasiveFunctionAO()
        retorno = PersuasiveFunctionAO(self.interactivity+other.interactivity, self.ludic+other.ludic, self.gamefication+other.gamefication)
    ...
    def s_diff(one,other):
        if not isinstance(other,PersuasiveFunctionAO) or not isinstance(one,PersuasiveFunctionAO):
            raise TypeError("ERROR: Trying to sum two different, non related objects")
        retorno={
    ...
    def get_function(external_id_):
        ...
        db_obj=PersuasiveFunctionDB.objects.filter(external_id=external_id_).first() # pylint: disable=no-member
        if not db_obj:
            raise RuntimeError("Error: No persistence data found for persuasive function id: "+external_id_)
    ...
    def save(self):
        ...
        if not self.external_id:
            raise RuntimeError("ERROR: Function was not properly created or is an operand!")
        ...
        try:
            ...
            raise RuntimeError("ERROR: Some of the persuasive function behaviors were not saved. ")
        #try saving the function
        try:
            PersuasiveFunctionDB.__persist__(self)
        except:
            raise RuntimeError("ERROR: Not able to save the Persuasive Function: "+self.external_id)
    ...
    def discard_interactivity_element(self, element):
        ...
        if element in self.interactivity:
            self.interactivity.discard(element)
            #need to check in the others to verify if remove from the set.
            if (not ((element in self.ludic) or (element in self.gamefication))):
                self.elements.discard(element)
    ...
    def discard_gamefication_element(self, element):
        ...
        if element in self.gamefication:
            self.gamefication.discard(element)
            #need to check in the others to verify if remove from the set.
            if (not ((element in self.interactivity) or (element in self.ludic))):
                self.elements.discard(element)
    ...
    def copy(self):
        ...
        """
        to_ret=PersuasiveFunctionAO(self.interactivity.copy(), self.ludic.copy(), self.gamefication.copy())
        return to_ret
    ...
    def issubset(self,other):
        ...
        Arguments:
            other {PersuasiveFunctionAO} -- The object to compare
        """
        if not isinstance(other,self.__class__):
            raise TypeError("ERROR: Operand Type differs. They must be the same")
